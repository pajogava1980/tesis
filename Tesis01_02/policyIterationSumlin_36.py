...
class PolicyIterationAgent:
    ...
    def eval_state_action(self, s, a):
        ...
        mat_tran = self.mat_tran_gen(correc_s, a, Y_reg_end)
        print(mat_tran)
...
#--------------------------------------------------------------------------------------------------------------------
    ...
    def matObj(self):
        ...
        try:
           ...
           # for intento in range(5):
            #    try:
             #       self.udp_socket.recvfrom(4)  # Intentar leer datos para limpiar el buffer
              ## except socket.timeout:
                #    print(f"Buffer vacío después de {intento + 1}/5 intentos.")
                 #   break
    ...
    def limpiar_buffer(self):
        ...
        """
        max_intentos_limpiar = 5
        intentos_exitosos = 0
        for i in range(max_intentos_limpiar):
            try:
                # Intenta leer datos del buffer
                self.udp_socket.recvfrom(4)
                intentos_exitosos += 1
                print(f"Intento {i+1}/{max_intentos_limpiar}: Dato eliminado del buffer.")
            except socket.timeout:
                print(f"Buffer vacío después de {i+1}/{max_intentos_limpiar} intentos. {intentos_exitosos} datos eliminados.")
                break
    ...
    Returns:
        ...
        for accion in range(self.nA):
            ...
            intentos = 0
            Y_reg_end_nuevo = None
            while Y_reg_end_nuevo is None and intentos < 3:
    ...
    def next_state(self, Y_reg_end):
        ...
        if Y_reg_end is None:
            ...
            if Y_reg_end == 0.927:                # Estado 0
                ...
                return 12
            elif 1.038 < Y_reg_end <= 1.048:      # Estado 13
                return 13
