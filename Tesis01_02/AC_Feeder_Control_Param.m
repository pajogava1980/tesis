...
pause(T/2)
while k<=N
    set_param('AC_Feeder_Control/Clk','Value','clk')
    pause(T/2)
    clk=~clk;
    if ~clk
        n=size(matObj.Vreg);
        Y(k)=matObj.Vreg(2,n(1,2))
        t(k)=k*T
    end
    pause(T/2)
    if clk
        tap=randi([-20 20],1,1);
        set_param('AC_Feeder_Control/Tap','Value','tap')
    k=k+1;
    end
end
set_param('AC_Feeder_Control', 'SimulationCommand', 'stop')
plot(t,Y)
