...
def obtener_entidades(df):
    return sorted(df[df["tipo_negociacion"] == "Venta"]["Nombres"].unique())
    return sorted(df[df["tc_venta"].notna()]["Nombres"].unique())
...
def calcular_spread_cliente(fecha, moneda, nombre, df, promedio_ponderado):
    ...
    """Calcula spread_cliente usando tc_venta de cualquier entidad"""
    fecha_filtro = datetime.strptime(fecha, "%Y-%m-%d").date()
    df_filtrado = df[(df["fecha"] == fecha_filtro) & (df["moneda"] == moneda) & (df["tipo_negociacion"] == "Venta") & (df["Nombres"] == nombre)].copy()
    ...
    df_filtrado["Promedio_Ponderado"] = promedio_ponderado
    return df_filtrado[["tc_venta", "spread_cliente", "Promedio_Ponderado"]]
...
def ejecutar_spread_cliente():
    ...
    resultado_df = calcular_spread_cliente(fecha, moneda, combo_nombre.get(), df, promedio_ponderado_actual)
    if resultado_df is not None and not resultado_df.empty:
        resultado_df["Promedio_Ponderado"] = promedio_ponderado_actual
    resultado_df["Ganancia"] = resultado_df["spread_cliente"] + float(entry_valor_euro.get())
...
# Mostrar el valor actual del EURO
tk.Label(root, text="Ingrese Valor del EURO:").pack()
entry_valor_euro = tk.Entry(root)
entry_valor_euro.pack()

# Obtener el valor del EURO
def actualizar_valor_euro():
    df_euro = df[(df["moneda"] == "EURO") & (df["Nombres"] == "COMMERZBANK AG")]
    if not df_euro.empty:
        promedio_euro = df_euro["tc_compra"].mean()
        entry_valor_euro.delete(0, tk.END)
        entry_valor_euro.insert(0, f"{promedio_euro:.4f}")
    else:
        entry_valor_euro.delete(0, tk.END)
        entry_valor_euro.insert(0, "No disponible")

actualizar_valor_euro()

# Agregar selección de entidad para calcular el spread_cliente
tk.Label(root, text="Entidad para Spread Cliente:").pack()
combo_nombre = ttk.Combobox(root, values=obtener_entidades(df), state='normal')

def filtrar_entidades(event):
    texto_ingresado = combo_nombre.get().upper()
    entidades_filtradas = [nombre for nombre in obtener_entidades(df) if texto_ingresado in nombre.upper()]
    combo_nombre['values'] = entidades_filtradas
