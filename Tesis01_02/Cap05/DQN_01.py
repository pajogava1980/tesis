# -*- coding: utf-8 -*-
"""
Created on Mon May 13 16:01:01 2024
...
from utils.atari_wrappers import make_env #Estoy importando desde otro WrapperClass.py
from utils.experienceBuffer import ExperienceBuffer, scale_frames

gym.logger.set_level(40) #Se deshabilita todos los mensajes, menos el de error
current_milli_time = lambda: int(round(time.time() * 1000)) #Función para medir el tiempo de ejecución 

def cnn(x):
    '''
    filters     Número de filtros-->kernels que se aplican en la capa 
    kernel_size Tamaño del filtro en la ventana de convolución 
    strides     Como se mueve el filtro, 4 pixeles horizaontal-vertical
    padding='valid'  No se aplica ningún relleno

    Parameters
    ----------
    x : TYPE
        DESCRIPTION.Es el tensor de entrada de la NN convolucional, estan los 
        datos que se propagaran hacia adelante a través de la red

    Returns
    -------
    x : TYPE
        DESCRIPTION.Se devuelve el tensor x, que representa la salida del CNN
        despues de la fase de de convolución y aplanamiento.

    '''
    #Primera capa convolucional
    x = Conv2D(filters=16, kernel_size=8, strides=4, padding='valid', activation='relu')(x)
    
    #Segunda capa convolucional 
    x = Conv2D(filters=32, kernel_size=4, strides=2, padding='valid', activation='relu')(x)
    
    #Tercera capa convolucional 
    x = Conv2D(filters=32, kernel_size=3, strides=1, padding='valid', activation='relu')(x)
    
    #Convierto en un vectro unidimencional. 
    x = Flatten()(x)
    return x
...
def fnn(x, hidden_layers, output_layer, activation=tf.nn.relu, last_activation=None):
    ...
    ----------
    x : TYPE
        DESCRIPTION. EL tensor de entrada de la NN
    hidden_layers : TYPE
        DESCRIPTION.Contiene el número de neuronas en cada capa oculta, ejemplo
        '[128]'-->una capa oculta con 128 nueronas
    output_layer : TYPE
        DESCRIPTION.Numero de neuronas de la capa de salida
    activation : TYPE, optional
        DESCRIPTION. The default is tf.nn.relu.
    last_activation : TYPE, optional
        DESCRIPTION. The default is None. Funcion de activacion en la capa de 
        salida de la NN

    Returns
    -------
    TYPE
        DESCRIPTION. Units=output_layer-->numero de agentes en action
...
def qnet(x, hidden_layers, output_size, fnn_activation=tf.nn.relu, last_activation=None):
    '''
    Llama a la funcion cnn(x) para pasarle los datos de entrada 'x' a través
    de la CNN . Esto hace que los datos de entrada sean relevantes en el aná-
    lisis

    Parameters
    ----------
    x : TYPE
        DESCRIPTION.Tensor de entrada de la DQN, contiene los datos que se
        propagarán hacia adelante a través de la red. 
    hidden_layers : TYPE
        DESCRIPTION. Lista de enteros-->número de unidades en cada capa oculta
        de la FNN
    output_size : TYPE
        DESCRIPTION. Es el numeor de enlaces de la FNN, corresponde al numero
        de acciones (OJO OLTC, BC)
    fnn_activation : TYPE, optional
        DESCRIPTION. The default is tf.nn.relu.
    last_activation : TYPE, optional
        DESCRIPTION. The default is None.

    Returns:
        Llama a la función fnn, para pasarle los datos a través de
        FNN. Esta función aplica capas densas totalmente conectadas con las 
        unidades y funciones de activación
    -------
    TYPE
        DESCRIPTION. Devuelve la salida de la FNN, que representa los valores
        de action estimados para cada posible accion en el entorno. 
