...
class PolicyIterationAgent:
    ...
    def eval_state_action(self, Y_reg, a):
        ...
        # Aplico cálculos paralelos
        with concurrent.futures.ThreadPoolExecutor() as ejecutor:

            # Tareas paralelas
            future_Y_reg = ejecutor.submit(self.get_Y_reg)  # Cálculo de Y_reg
            future_Y_reg_end = ejecutor.submit(self.get_Y_reg_end)  # Cálculo de Y_reg_end

            # Obtener los resultados de las tareas
            Y_reg = future_Y_reg.result()
            Y_reg_end = future_Y_reg_end.result()
    ...
    #--------------------------------------------------------------------------------------------------------------------
    #--------------------------------------------------------------------------------------------------------------------
    def get_Y_reg_end(self):
        """
        Cálculo de Y_reg_end
        """
        return self.matObj()
    ...
    def get_tap_position(self, Y_reg, a):
        ...
        if Y_reg < desired_min:
            ...
            if a == 1:
                ...
                posicion_faltante = [pos for pos in posible_posicion if pos > pos_tap_actual]
            else:
            ...
            # Cuando el Y_reg está en el rango deseado
            posicion_faltante = posible_posicion
    ...
    def next_state(self, Y_reg_end):
        ...
        """
        print(f"Y_reg_end recibido del Y_reg_end de transición: {Y_reg_end}")
    ...
    def calculo_reward(self, Y_reg_end):
        ...
        if Y_reg_end is None or Y_reg_end < 0.95:
            ...
            #Recompenza escalonada basada en que tancerca esta el voltaje del valor nominal
