...
class PolicyIterationAgent:
    ...
    def get_tap_position(self, Y_reg, a):
        ...
        # Mecanismo de amortiguación: si estamos dentro del rango deseado
        if self.desired_min <= Y_reg <= self.desired_max:
            # Si estamos cerca del valor ideal (dentro del rango 0.98 - 1.02)
            if 0.98 <= Y_reg <= 1.02:
                amortiguacion_factor = 0.5  # Reducir el cambio a la mitad
            else:
                amortiguacion_factor = 0.75  # Reducir el cambio en un 25%
        else:
            amortiguacion_factor = 1.0  # Sin amortiguación si está fuera del rango

        if a == 0:
            cambio_tap = np.random.choice([-2, -1, 0]) * amortiguacion_factor
        elif a == 1:
            cambio_tap = np.random.choice([0, 1, 2]) * amortiguacion_factor
        else:
            cambio_tap = 0

        nueva_pos_tap = pos_tap_actual + cambio_tap
