...
#--------------------------------------------------------------------------------------------------------------------
    ...
    Returns:
        ...
        #paso 2: Definicion parametros clave
        tap_original = float(self.eng.workspace['tap']) #Guardo el TAP original antes de remplazarlo
        acciones_tap = {0: -1, 1: 0, 2: +1}             #bajar (-1), mantener (0), subir (+1)
        objetivo = 0.995                                #Punto medio entre 0.99-1.00
        distancia_acciones = {}                         # Diccionario donde voy a almacenar la diferencia de las distancias de cada acción

        # Paso 3: Evaluación de cada accion Posible
        for accion in range(self.nA):
            nuevo_tap = max(self.pos_min_tap, min(self.pos_max_tap, tap_original + acciones_tap[accion]))
            ...
            if Y_reg_end_nuevo is None:
                print("Error: No se recibió un valor válido de `Y_reg_end_nuevo` después de 3 intentos.")
                return [(1.0, s, -10, True)]

            distancia_acciones[accion] = abs(Y_reg_end_nuevo - objetivo) #Para determinar cual es la mejor acción y ver si se mantiene si esta en rango nominal 

            next_s = self.next_state(Y_reg_end_nuevo)
            done = self.is_terminal_state(next_s)
            rew = self.calculo_reward(Y_reg_end_nuevo)

            self.P[correc_s][accion] = (next_s, rew, done, Y_reg_end_nuevo) # Almacena temporalmente la información

        #Paso 5: Selección de la mejor acción 
        ...
        assert abs(sum(probabilidad.values()) - 1.0) < 1e-6, "Error: La suma de probabilidades no es 1.0"

        #Paso 6: Creación de la matriz de transición
        # Creo mi matriz de transición
        transiciones = []
        for accion in range(self.nA):
            next_s, rew, done, Y_reg_end_nuevo = self.P[correc_s][accion]
            transiciones.append((probabilidad[accion], next_s, rew, done))
            print(f"Acción: {accion}, TAP: {tap_original + acciones_tap[accion]}, Y_reg_end_nuevo: {Y_reg_end_nuevo},"f" Distancia al objetivo: {distancia_acciones[accion]:.4f}, Probabilidad: {probabilidad[accion]:.2f}")

        #Restaurar TAP original
        self.eng.workspace['tap'] = tap_original
        self.eng.eval("set_param('AC_Feeder_Control/Tap','Value','tap')", nargout=0)
        print(f"[SYNC] TAP restaurado a {tap_original} en Simulink tras mat_tran_gen.")
    ...
    def next_state(self, Y_reg_end):
        ...
        if Y_reg_end is None:
            ...
            if Y_reg_end == 0.927:                # Estado 0
                ...
                return 5
            elif 0.973 < Y_reg_end <= 0.983:      # Estado 6
                return 6
            elif 0.983 < Y_reg_end <= 0.992:      # Estado 7
                return 7
            elif 0.992 < Y_reg_end <= 1.002:      # Estado 8
                return 8
            elif 1.002 < Y_reg_end <= 1.010:      # Estado 9
                return 9
            elif 1.010 < Y_reg_end <= 1.020:      # Estado 10
                return 10
            elif 1.020 < Y_reg_end <= 1.029:      # Estado 11
                return 11
            elif 1.029 < Y_reg_end <= 1.039:      # Estado 12
                return 12
            elif 1.039 < Y_reg_end <= 1.048:      # Estado 13
                return 13
            elif 1.048 < Y_reg_end <= 1.057:      # Estado 14
                return 14
            elif 1.057 < Y_reg_end <= 1.067:      # Estado 15
                return 15
            elif 1.067 < Y_reg_end <= 1.075:      # Estado 16
                return 16
            elif 0.918 <= Y_reg_end < 1.075:      # Estado 17
                return 17
            elif 0.908 <= Y_reg_end < 0.918:      # Estado 18
                return 18
            elif 0.899 <= Y_reg_end < 0.908:      # Estado 19
                return 19
            elif 0.890 <= Y_reg_end < 0.899:      # Estado 20
                return 20
            elif 0.880 <= Y_reg_end < 0.890:      # Estado 21
                return 21
            elif 0.871 <= Y_reg_end < 0.880:      # Estado 22
                return 22
            elif 0.863 <= Y_reg_end < 0.871:      # Estado 23
                return 23
                ...
                return 27
            elif 0.815 <= Y_reg_end < 0.825:      # Estado 28
                return 28
            elif 0.806 <= Y_reg_end < 0.815:      # Estado 29
                return 29
            elif 0.797 <= Y_reg_end < 0.806:      # Estado 30
                return 30
            elif 0.788 <= Y_reg_end < 0.797:      # Estado 31
                return 31
            elif 0.785 <= Y_reg_end < 0.788:      # Estado 32
                return 32
            else:
                print("Y_reg_end fuera de rango. Regresando al estado inicial.")
                return 33  # Estado por defecto en caso de que no cumpla ningún rango
    ...
    def calculo_reward(self, Y_reg_end):
        ...
        if Y_reg_end is None:
            ...
            if   0.779 <= Y_reg_end < 0.788:
                return 0
            elif 0.788 <= Y_reg_end < 0.797:
                return 0
            elif 0.797 <= Y_reg_end < 0.806:
                return 0
            elif 0.806 <= Y_reg_end < 0.815:
                return 0
            elif 0.815 <= Y_reg_end < 0.825:
                return 0
            elif 0.825 <= Y_reg_end < 0.834:
                return 0
            elif 0.834 <= Y_reg_end < 0.844:
                return 0
            elif 0.844 <= Y_reg_end < 0.853:
                return 0
            elif 0.853 <= Y_reg_end < 0.863:
                return 0
            elif 0.863 <= Y_reg_end < 0.871:
                return 0
            elif 0.871 <= Y_reg_end < 0.881:
                return 0
            elif 0.880 <= Y_reg_end < 0.890:
                return 0
            elif 0.890 <= Y_reg_end < 0.899:
                return 0
            elif 0.899 <= Y_reg_end < 0.918:
                return 0
            elif 0.918 <= Y_reg_end < 0.927:
                return 0
            elif 0.927 <= Y_reg_end < 0.936:
                return 0
            elif 0.936 <= Y_reg_end < 0.945:
                return 0
            elif 0.945 <= Y_reg_end < 0.955:
                return 0
            elif 0.955 <= Y_reg_end < 0.964:
                return 1
            elif 0.964 <= Y_reg_end < 0.973:
                return 2
            elif 0.973 <= Y_reg_end < 0.983:
                return 3
            elif 0.983 <= Y_reg_end < 0.992:
                return 4
            elif 0.992 <= Y_reg_end < 1.002:
                return 5
            elif 1.002 <= Y_reg_end < 1.011:
                return 4
            elif 1.011 <= Y_reg_end < 1.02:
                return 3
            elif 1.02 <= Y_reg_end < 1.029:
                return 2
            elif 1.029 <= Y_reg_end < 1.038:
                return 1
            elif 1.038 <= Y_reg_end < 1.048:
                return 1
            elif 1.048 <= Y_reg_end < 1.057:
                return 1
            elif 1.057 <= Y_reg_end < 1.067:
                return 0
            elif 1.067 <= Y_reg_end < 1.750:
                return 0
            elif 1.750 <= Y_reg_end:
                return -10
    #--------------------------------------------------------------------------------------------------------------------
    #--------------------------------------------------------------------------------------------------------------------
    def is_terminal_state(self, next_state):
        ...
        return next_state <= 0.85 or next_state >= 1.075 or (0.99 <= next_state < 1.01)
    #--------------------------------------------------------------------------------------------------------------------
    #--------------------------------------------------------------------------------------------------------------------
