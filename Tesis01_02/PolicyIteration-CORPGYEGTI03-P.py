...
RL-Puede formularce como un Markov Decision Process (MDP).
    ...
    ----------------------------------------------
        Escoje la A_t en un S_t, y pueden ser:
    ...
    Con la definición de RETURN, se puede definir el Objetivo de encontrar la PI óptima que MAX RETURN.
        PI Optima, es un S_t y A_t ótimos
                ...
                Puede ser analizado por trayectorias:
                    ...
                    - Finitas Soluciones
                        ...
                        El uso -->lammda incrementa la estabilidad del algorítmo
                            ...
                            lammda[0.9-0.999]
                            lammda=--> 1
                                SI no se tiene apuro.
                            lammda=--> 0
                                Se quiere un resultado ya.
    --------------------------------------------
    | 1.3. Value Function V_(PI)(S)- V-Function |
    --------------------------------------------
    Proporciona un indicador de CALIDAD
        Necesario para que la POLICY (PI) escoja la MEJOR --> A_(t+1)
            PI solo debe elejir la A_t que resulta en el S_(t+1) con la + ALTA CALIDAD
            Esta ALTA CALIDAD lo proporciona V-FUNCTION
                Se estima la QUALITY en terminos del valor RETURN esperado
                    Dependiendo de un S_t seguido de una PI-->que escoje una A_t y un S_t
        V-Function puede definirse en terminos de Q-Function, usando el Q*
            Obtengo V*(s) = max_(A_t)Q*(S_t, A_t)
                a*(s) = argmax_(A_t)Q*(S_t,A_t)
    --------------------------------------------------------------
    | 1.3.1 Action Value Function Q_(PI)(S_t, A_t)--> Q-Function |
    --------------------------------------------------------------
    Retorna el valor esperado de un S_t tambien condicionado de la primera A_t.
    Se lo conoce como Action-Value Function
    Q*--> Optimal
...
def eval_state_action(V, s, a, gamma = 0.99):
    ...
    #print("probabilidad de transitar:", env.p, "next_s ", env.next_s, "rew:", env.rew )
    pepi = np.sum([p * (rew + gamma*V[next_s]) for p, next_s, rew, _ in env.P[s][a]])
    #print(pepi)
    return pepi
