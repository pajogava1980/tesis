...
class PolicyIterationAgent:
    ...
    def get_tap_position(self, Y_reg, a):
        ...
        if Y_reg < 0.95:
            base_tap_position = self.pos_max_tap  #Significativamente bajo voltaje
        elif Y_reg > 1.05:
            base_tap_position = self.pos_min_tap   #Suave bajo el voltaje
        else:
            base_tap_position = 0

        # Ajuste de la posición del TAP con A_t
        if a == 0:  #A_t 0: Bajar el TAP
            tap_position = base_tap_position - 1
        ...
        tap_position = max(self.pos_min_tap, min(sensitivity, self.pos_max_tap))
...
if __name__ == '__main__':
    ...
    agent = PolicyIterationAgent(nS=5, nA=2, gamma=0.9, eps=0.5, eng=eng)  # Inicializo el agent
    ...
    print("\n La matriz de la politica PI es: ", agent.policy.reshape((5, 1)))
