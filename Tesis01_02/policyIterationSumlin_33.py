...
class PolicyIterationAgent:
    #--------------------------------------------------------------------------------------------------------------------
    #--------------------------------------------------------------------------------------------------------------------
    def __init__(self, nS, nA, gamma, eps, eng, prob_satis = 0.9, prob_falla = 0.7):
        ...
        self.udp_socket.bind((self.ip, self.port))  # Puerto y dirección IP del agente
    ...
    #--------------------------------------------------------------------------------------------------------------------
    def matObj(self):
        """ Rx datos desde Simulink a través del socket UDP, decodificarlos y usar el valor obtenido Y_reg_end para realizar 
        cálciulos. 
        Returns:
            data--> Leer 4 bytes de data, 
            addr--> Dirección del remitente
            struct--> Se usa este módulo para interpretar los 4 bytes cómo npumero float 
            <f--> Indica que es un flotante de formato little-endian
        self.ip = '127.0.0.1'
        self.port = 9091
        self.udp_socket = socket.socket(socket.AF_INET, socket.SOCK_DGRAM)
        self.udp_socket.bind((self.ip, self.port))  # Puerto y dirección IP del agente
        ...
        print(f"Esperando datos en {self.ip}:{self.port}...")
