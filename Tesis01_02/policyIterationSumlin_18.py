...
class PolicyIterationAgent:
    ...
    def policy_evaluation (self):
        alpha = 0.01    #Para suavizar los cambios
        iteracion = 0
        delta_values = []
        max_iteracion = 10
        iteracion_no_down = 0
        ...
        while iteracion < max_iteracion:             # Formula 3.8 pag. 62, pseudocode pg. 64 "miesntras PI no es estable"
            ...
            for s in range(self.nS): # Lazo para todos los estados nS = 16-->cuadriculas estados de observación
                ...
                #self.V[s] = reward + alpha *(reward + self.V[s] - old_v)
    ...
    def policy_improvement(self):
        ...
        for s in range(self.nS):
            #Acción anterior es tomada del array de la policy en la ubicación 's'
            old_a = self.policy[s]
            action_values = []
    ...
    def get_tap_position(self, Y_reg, a):
        ...
        #tap_steps = 33          # Número total de pasos del TAP
        #max_reg_perce = 0.10    # Máxima regulación permitida (±10%)
        #sensitivity = (max_reg_perce * 2)/(tap_steps - 1)

        # Rango deseado de voltaje
        ...
        # Monitore de todas las posiciones de forma aleatoria sin repetirse
        ...
        #Elijo una posición al azar entre las no exploradas
        #posicion_faltante = list(set(posible_posicion) - self.explored_positions)
        #tap_position = np.random.choice(posicion_faltante)
