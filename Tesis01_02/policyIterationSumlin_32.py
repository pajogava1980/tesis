...
class PolicyIterationAgent:
    ...
    def __init__(self, nS, nA, gamma, eps, eng):
        ...
        self.udp_socket.bind(('127.0.0.1', 9091))  # Puerto y dirección IP del agente
        self.udp_socket.settimeout(5)  # Configura un tiempo de espera
    ...
    def eval_state_action(self, s, a):
        ...
        transicion = self.mat_tran_gen(Y_reg_end)

        # Tupla (p, next_s, rew, done)
        for p, next_s, rew, done in transicion:
            if done:
                v_fun += p * rew
            else:
    ...
    #--------------------------------------------------------------------------------------------------------------------
    #--------------------------------------------------------------------------------------------------------------------
    def mat_tran_gen(self, Y_reg_end):
        """Se genera la matriz de transición general.

        Args:
            Y_reg_end (_type_): _description_
        """
        ...
        for s in range(self.nS):
            ...
            for a in range(self.nA):
                ...
                rew = self.calculo_reward(Y_reg_end)

                prob_min = 0.7 if rew < 0 else 0.9
                prob_alt = 1.0 - prob_min           #alt = alternativa entre 0.3 o 0.1 según el rewad

                self.P[s][a] = [
                    (prob_min, next_s, rew, done),
                    (prob_alt, s, -1, False)
                ]
        return self.P[s][a]
    ...
    def matObj(self):
        """
        Obtengo los valores desde Simulink para Y_reg y Y_reg_end. la comunicación en el otro sentido.
        self.udp_socket = socket.socket(socket.AF_INET, socket.SOCK_DGRAM)
        self.udp_socket.bind(('127.0.0.1', 9090))

        """
        print(f"Esperando datos en 127.0.0.1:9091...")

        #Configuración de la IP local, mi laptop
"""         ip = "127.0.0.1"    #Dirección IP del Servidor 
        puerto = 9091       #Puede ser cualquier puerto disponible
        ...
        while True:
            try:
                ...
                data, addr = self.udp_socket.recvfrom(4)  # Leer 4 bytes
                ...
                # Convertir Vreg a pu utilizando la base de tensión
                Y_reg_end = vreg_redondeado / self.V_base_fase
                Y_reg_end = round(Y_reg_end, 3)
                print(f"Valor de 'Y_reg_end' en pu: {Y_reg_end}")
        ...
        """ if not hasattr(self, 'udp_socket') or self.udp_socket is None:
            try:
                self.udp_socket = socket.socket(socket.AF_INET, socket.SOCK_DGRAM)
                self.udp_socket.bind((ip, puerto))
                #self.udp_socket 
                #self.udp_socket.bind
                print(f"Servidor UDP iniciado en {self.udp_socket.bind}")
            except OSError as e:
                print(f"Error al iniciar el servidor UDP: {e}")
                return None

        try:
            print("Esperando datos desde Simulink...")

            #self.udp_socket.setsockopt(socket.SOL_SOCKET, socket.SO_RCVBUF, 0)    #Limpia el buffer
            data, _ = self.udp_socket.recvfrom(4)  # Esperar un paquete
            

            # IMPRIME LOS DATOS CRUDOS RECIBIDOS
            print(f"Datos crudos recibidos: {data}")

            if len(data) >= 4:  # Asegurarse de tener suficientes bytes para un float
                vreg_actual = struct.unpack('<f', data[:4])[0]  # Decodificar valor flotante
                print(f"Valor de Vreg recibido: {vreg_actual}")

                # Convertir Vreg a pu utilizando la base de tensión
                Y_reg_end = vreg_actual / self.V_base_fase
                Y_reg_end = round(Y_reg_end, 3)
                print(f"Valor de 'Y_reg_end' en pu: {Y_reg_end}")

                return Y_reg_end
            else:
                print("Advertencia: Paquete recibido no tiene el tamaño esperado.")
                return None
        
        except socket.timeout:
            print("Error: Tiempo de espera agotado al recibir datos UDP.")
            return None

        except struct.error as e:
            print(f"Error al recibir o decodificar datos UDP: {e}")
            return None
    ...
    def get_tap_position(self, a):
        ...
        cambios = [-2, +2, -1, +1] #a = [0, 1, 2, 3]
        cambio_tap = pos_tap_actual + cambios[a]
        #cambio_tap = pos_tap_actual + (1 if a == 1 else -1 if a == 0 else 0) # Operador ternario

        nueva_pos_tap = max(self.pos_min_tap, min(self.pos_max_tap, cambio_tap))
        # Cómo poner una saturación para el TAP??
        # Para que el control no se salga de control.. debo avisarle que el sistema que no debe bajar o subir de una posición
        # AP..un Reward super negativo de cuanto?? analizar
    ...
    def next_state(self, Y_reg_end):
        ...
        if Y_reg_end is None:
            ...
            Y_reg_end = round(Y_reg_end, 3)
            if Y_reg_end < 0.850:          # Voltaje extremadamente bajo Falla, Si es un problema de mala regulacion del TAP, el TAP debería poder regular, pero
                #SI esta en falla, 0.85 intervalos mas pequeños, para analizarlo.
                return 0
            elif 0.850 <= Y_reg_end < 0.900:   # Voltaje muy bajo
                return 1
            elif 0.900 <= Y_reg_end < 0.930:   # Voltaje bajo
                return 2
            elif 0.930 <= Y_reg_end < 0.950:  # Voltaje ligeramente bajo
                return 3
            elif 0.950 <= Y_reg_end < 0.970:  # Voltaje moderadamente bajo
                return 4
            elif 0.970 <= Y_reg_end <= 0.990: # Voltaje nominal
                return 5
            elif 0.990 <= Y_reg_end <= 1.000:  # Voltaje nominal
                return 6
            elif 1.00 < Y_reg_end <= 1.010:  # Voltaje ligeramente alto
                return 7
            elif 1.010 < Y_reg_end <= 1.030:   # Voltaje alto
                return 8
            elif 1.030 < Y_reg_end <= 1.050:   # Voltaje alto
                return 9
            elif 1.050 < Y_reg_end <= 1.070:   # Voltaje alto
                return 10
            elif 1.070 < Y_reg_end <= 1.100:   # Voltaje alto
                return 11
            else:
                return 12                    # Voltaje extremadamente alto, físicamente significa una mala regulación de voltaje, en SE, el TAP debe funcionar, A=0, baja el TAP
    ...
    def calculo_reward(self, Y_reg_end):
        ...
        if Y_reg_end is None:
            ...
            Y_reg_end = round(Y_reg_end, 3)
            # Penalizaciones y recompenzas finas
            if Y_reg_end < 0.850:
                reward = -10
            elif 0.850 <= Y_reg_end < 0.900:
                reward = -8
            elif 0.900 <= Y_reg_end < 0.930:
                reward = -5
            elif 0.930 <= Y_reg_end < 0.950:
                reward = -3
            elif 0.950 <= Y_reg_end <= 0.970:
                reward = -1
            elif 0.970 <= Y_reg_end < 0.990:
                reward = 2
            elif 0.990 <= Y_reg_end <= 1.000:
                reward = 5
            elif 1.000 < Y_reg_end <= 1.010:
                reward = 5
            elif 1.010 < Y_reg_end <= 1.030:
                reward = 2
            elif 1.030 < Y_reg_end <= 1.050:
                reward = -1
            elif 1.050 < Y_reg_end <= 1.070:
                reward = -3
            elif 1.070 < Y_reg_end <= 1.100:
                reward = -5
            else:
                reward = -10
...
if __name__ == '__main__':
    ...
    agent = PolicyIterationAgent(nS = 13, nA = 4, gamma = 0.88, eps = 0.01, eng=eng)  # Inicializo el agent
