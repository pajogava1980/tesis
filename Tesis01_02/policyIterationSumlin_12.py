...
class PolicyIterationAgent:
    ...
    def policy_evaluation (self):
        ...
        while True:             # Formula 3.8 pag. 62, pseudocode pg. 64 miesntras PI no es estable
            ...
            for s in range(self.nS): # Lazo para todos los estados nS = 16-->cuadriculas estados de observación
                ...
                #Actualizo mi gamma 
                dynamic_gamma = self.update_gamma(Y_reg, iteracion)
                ...
                #factor_amortiguamiento = 0.9
                #delta *= factor_amortiguamiento
    ...
    def get_tap_position(self, Y_reg, a):
        ...
        #Posiciones base del TAP de forma dinámica
        if Y_reg < 0.9:
            sensitivity = 5
        elif Y_reg < 0.95:
            sensitivity = 3  #Significativamente bajo voltaje
        elif 0.95 <= Y_reg <= 1.05:
            sensitivity = 1   #Suave bajo el voltaje
        elif Y_reg > 1.05 and Y_reg <= 1.1:
            sensitivity = 2
        else:
            sensitivity = 5

        # Posición del TAP con base en el Voltaje
        if Y_reg < 0.95:
            base_tap_position = sensitivity #Bajo voltaje
        elif Y_reg > 1.05:
            base_tap_position = -sensitivity    #Alto voltahje
        else:
            base_tap_position = 0   #Rango aceptable
    ...
    def run_episodes(self, num_games, Y_reg_init, max_steps):
        ...
        for game_num in range(num_games):  # Use game_num as part of the filename
            ...
            if step >= max_steps:
                print(f"Episode terminated after reaching {max_steps} steps.")
            
            # Generate a unique filename using the game number and timestamp
            filename = f"simulation_result_game_{game_num}_{timestamp}.png"
        ...
        print(f'Completed {num_games} episodes, total reward: {tot_rew}')
        return action_taken
    #--------------------------------------------------------------------------------------------------------------------
    #--------------------------------------------------------------------------------------------------------------------
...
if __name__ == '__main__':
    ...
    max_steps = 20  # Setear el máximo numero de pasos 
    ...
    for Y_reg_init in random_Y_reg_init_values:
        ...
        actions = agent.run_episodes(10, Y_reg_init=Y_reg_init, max_steps=max_steps)
