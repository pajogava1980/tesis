...
from sympy.abc import x, y, r, s, a, t
...
class PolicyIterationAgent:
    ...
    def eval_state_action(self, Y_reg, a):
        ...
        eng.workspace['tap'] = float(tap_position.item())
        ...
        #Calculo de la penalidad variable a mayor desviación, > penalidad. 
    ...
    def policy_evaluation (self):
        """
        No uso las probabilidades ya que mi sistema es deterministico, la rpobabilidad de transición
        es 1
        """
        max_iteracion = 40
        iteracion = 0
        ...
        while iteracion < max_iteracion:             # Formula 3.8 pag. 62, pseudocode pg. 64 "miesntras PI no es estable"
            delta = 0           # la V_función es etable cuando delta sea < que eps
            for s in range(self.nS): # Para cada estado
                old_v = self.V[s]
                v_funtion = np.random.rand()   # Paso 1: Inicializo con un valor arbitrario V(s)
                for a in range(self.nA):    # Para cada acción
                    prob = 1
                    reward, Y_reg = self.eval_state_action(self.V[s], a)
                    sig_state = self.next_state(Y_reg)
                    # Nuevos V(s) utilizadno la Ec. de Bellman Ec. 3.8 
                    v_funtion += prob * (reward + self.gamma * self.V[sig_state])   # Paso 2: Actualizo V(s) con Bellman
    ...
    def get_tap_position(self, Y_reg, a):
        ...
        Returns:
            _type_: _description_
        """
        desired_min = self.desired_min  # 0.95 por defecto
        desired_max = self.desired_max  # 1.05 por defecto

        # Obtener la posición actual del TAP desde el workspace
        pos_tap_actual = eng.workspace['tap']

        # Crear una lista con todas las posiciones posibles del TAP
        posible_posicion = list(range(self.pos_min_tap, self.pos_max_tap + 1))

        # Monitore de todas las posiciones de forma aleatoria sin repetirse, usa la función explored_positions
        if not hasattr(self, 'explored_positions'):
            self.explored_positions = set() #Inicializo conjunto de posiciones exploradas

        if len(self.explored_positions) >= len(posible_posicion):
            self.explored_positions = set() #Reinicio el conjunto cuando se haya explorado

        # Donde decido subir o bajar el TAP según Y_reg y la acción 'a'
        if Y_reg < desired_min:
            # Si el voltaje es bajo, el TAP debería subir
            if a == 1:
                # Acción subir: Seleccionar posiciones mayores a la actual.
                posicion_faltante = [pos for pos in posible_posicion
                                       if pos > pos_tap_actual
                                       and pos not in self.explored_positions]
            else:
                # Acción contraria: Subir aunque debe bajar el TAP, pero de forma aleatoria
                posicion_faltante = [pos for pos in posible_posicion
                                       if pos > pos_tap_actual
                                       and pos not in self.explored_positions]
        elif Y_reg > desired_max:
            # Si el voltaje es alto, el TAP debería bajar
            if a == 0:
                # Acción para bajar el TAP: seleccionando posiciones menores a la actual
                posicion_faltante =[pos for pos in posible_posicion
                                      if pos < pos_tap_actual
                                      and pos not in self.explored_positions]
            else:
                # Acción contraria: Bajar aunque debería subir, pero de forma aleatoria
                posicion_faltante = [pos for pos in posible_posicion
                                       if pos < pos_tap_actual
                                       and pos not in self.explored_positions]
        else:
            #Cuando el voltaje Y_reg esta en el rango deseado
            if a == 1:
                posicion_faltante = [pos for pos in posible_posicion
                                       if pos > pos_tap_actual
                                       and pos not in self.explored_positions]
                ...
                posicion_faltante = [pos for pos in posible_posicion
                                       if pos < pos_tap_actual
                                       and pos not in self.explored_positions]
        # Cuando ya se haya explorado todas las posiciones del TAP
        if len(posicion_faltante) == 0:
            self.explored_positions = set()
            posicion_faltante = posible_posicion

        # Selecciono una posición aleatoria al azar
        tap_position = np.random.choice(posicion_faltante)
        
        # Añadio la posición elegida al conjunto de posciones exploradas
        self.explored_positions.add(tap_position)
        return tap_position
    #--------------------------------------------------------------------------------------------------------------------
    #--------------------------------------------------------------------------------------------------------------------
    ...
    #--------------------------------------------------------------------------------------------------------------------
    #--------------------------------------------------------------------------------------------------------------------
    def calculo_reward(self, s, a, s_next):
        ...
        elif Y_reg_end is None or Y_reg_end > 1.05:
    ...
    def choose_action(self, s, epsilon):
        if np.random.rand() < epsilon:
            # Explora una axión aleatoria
            return np.random.choice(self.nA)
        else:
            # Exploro la mejor A_t basada en la PI_t
