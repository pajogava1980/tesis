...
class PolicyIterationAgent:
    ...
    def eval_state_action(self, Y_reg, a):
        ...
        #eng.workspace['clk'] = True
        eng.workspace['tap'] = tap_position #Escribo el valor del TAP en el workspace
       #eng.tap_position(nargout=0)

        #self.eng.set_param('AC_Feeder_Control/Tap', 'Value', str(tap_position))

        #Corro el modelo de Simulink
        eng.eval("set_param('AC_Feeder_Control', 'SimulationCommand', 'start')", nargout=0)
        time.sleep(1)
        #eng.eval("set_param('AC_Feeder_Control/Tap', 'Value', str(tap_position)')")
        #eng.worspace['start']
        #self.eng.set_param('AC_Feeder_Control', 'SimulationCommand', 'start')
        ...
        #self.eng.pause(3)  # Pausa para sincronización de la simulación
        ...
        #file_path = 'C:\\Users\\pagav\\OneDrive - Escuela Superior Politécnica del Litoral\M12_TT\DRL\Tesis01_02\Register.mat'
        file_path = 'C:\\Users\\pablo.gamboa\\OneDrive - Escuela Superior Politécnica del Litoral\M12_TT\DRL\Tesis01_02\Register.mat'
        ...
        with h5py.File(file_path, 'r') as mat_file:
            ...
            mi_variable = mat_file['Vreg']
            
            # Convertir la variable a un array de NumPy
            mi_variable_np = mi_variable[:]
            
            # Obtener el último valor de la variable
            if mi_variable_np.size > 0:
                Y_reg_mat = mi_variable_np[0][-1]  # Último valor de un array 1D o 2D
                Y_reg_end_pu = Y_reg_mat/self.V_base_fase
                Y_reg_end = Y_reg_end_pu
                # Si es una matriz 2D, puedes acceder al último valor de la última fila y columna
                # ultimo_valor = mi_variable_np[-1, -1]
            else:
                print("La variable está vacía.")



        #matObj = self.eng.workspace['matObj']   # Obtengo datos de la simulación de 'matObj'
        #Y_reg_end = matObj.Vreg[0][-1]          # De cada iteración el útimo valor de Vreg

        # Paro la simulación
        self.eng.set_param('AC_Feeder_Control', 'SimulationCommand', 'stop')
...
if __name__ == '__main__':
    ...
    agent = PolicyIterationAgent(nS=16, nA=2, gamma=0.99, eps=0.01, eng=eng)
