...
weighted_avg = df.groupby(['fecha', 'moneda', 'tipo_negociacion']).apply(
    lambda x: (x['tipo_cambio'] * x['volumen']).sum() / x['volumen'].sum()
).reset_index()

# Renombrar la columna resultante
weighted_avg.columns = ['Fecha', 'Moneda', 'Tipo de Negociación', 'id', 'Promedio Ponderado']

# Guardar los resultados en un nuevo archivo Excel
output_file = 'resultado_por_tipo_operacion_02.xlsx'
